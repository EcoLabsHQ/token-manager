...
const InputField = ({
  ...
  return (
    <div className='flex flex-col gap-1.5 flex-1 min-w-0'>
      <label className='text-xs sm:text-sm text-gray-500 leading-relaxed'>
        {label}
      </label>
      <div
        className={`border border-gray-300 rounded-md flex items-center px-2 sm:px-2.5 py-1.5 sm:py-2 ${disabled ? 'bg-gray-50' : 'bg-white'}`}
      >
        <input
          type='text'
          value={displayValue}
        ...
        />
        {suffix && (
          <span className='text-xs sm:text-sm text-black ml-2 flex-shrink-0'>
            {suffix}
          </span>
        )}
      </div>
  ...
  return (
    <div className='flex flex-col gap-1 flex-1 min-w-0'>
      ...
      <div className='border border-gray-300 rounded-md flex items-center px-2 sm:px-2.5 py-1.5 sm:py-2 bg-white'>
        <span className='flex-1 text-xs sm:text-sm text-black font-mono truncate'>
          {value}
        </span>
        {onCopy && (
          <button
...
const SectionCard = ({ title, children }: SectionCardProps) => (
  <div className='border border-gray-200 rounded-xl sm:rounded-2xl p-3 sm:p-5 flex flex-col gap-3 sm:gap-4'>
    <h4 className='text-xs sm:text-sm font-semibold text-black tracking-tight'>
      {title}
    </h4>
    {children}
  </div>
  ...
  <div className='bg-red-50 border border-red-200 rounded-lg p-3 flex items-start gap-2'>
    ...
    <div>
      ...
      <p className='text-red-600 text-xs mt-0.5'>
        All token operations are disabled while the contract is paused.
      </p>
    </div>
  </div>
);
...
const MintSection = ({
  ...
  return (
    <SectionCard title='Mint'>
      ...
      {error && <p className='text-red-500 text-xs sm:text-sm'>{error}</p>}
      {success && (
        <p className='text-green-500 text-xs sm:text-sm'>Mint successful!</p>
      )}
      <ActionButton
        label='Mint'
        onClick={handleMint}
        disabled={!isValid || !isOwner || isPaused}
        loading={isLoading}
      />
    </SectionCard>
  );
...
const BurnSection = ({
  ...
  return (
    <SectionCard title='Burn'>
      ...
      {error && <p className='text-red-500 text-xs sm:text-sm'>{error}</p>}
      {success && (
        <p className='text-green-500 text-xs sm:text-sm'>Burn successful!</p>
      )}
      <ActionButton
        label='Burn'
        onClick={handleBurn}
  ...
  const modalSteps = [
    {
      ...
      description: isPaused
        ? 'Unpausing contract on L1'
        : 'Pausing contract on L1',
      chain: 'ethereum' as const,
    },
    {
      title: 'Celo (L2)',
      description: isPaused
        ? 'Unpausing contract on L2'
        : 'Pausing contract on L2',
      chain: 'celo' as const,
    },
  ];
    ...
    <SectionCard title='Pause Contract'>
      ...
      {isEthereumEnabled && (
        <p className='text-xs text-blue-600 bg-blue-50 p-2 rounded-lg'>
          ℹ️ This is an Ethereum Enabled token. Pause/Unpause will be applied to
          both L1 and L2 tokens.
        </p>
      )}
      ...
      {isEthereumEnabled ? (
        <div className='space-y-2'>
          <div
            className={`px-3 py-2 rounded-lg text-xs sm:text-sm font-medium flex justify-between items-center ${isPaused ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}
          >
            <span>L1 (Ethereum):</span>
            <span>{isPaused ? 'Paused' : 'Active'}</span>
          </div>
          <div
            className={`px-3 py-2 rounded-lg text-xs sm:text-sm font-medium flex justify-between items-center ${l2IsPaused ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}
          >
            <span>L2 (Celo):</span>
        ...
        <div
          className={`px-3 py-2 rounded-lg text-xs sm:text-sm font-medium ${isPaused ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}
        >
...
const BridgeSection = ({
  ...
  const [amount, setAmount] = useState('');
  const [direction, setDirection] = useState<'l1-to-l2' | 'l2-to-l1'>(
    'l1-to-l2',
  );
  const [error, setError] = useState<string | null>(null);
  const [success, setSuccess] = useState(false);
  ...
  const [selectedWithdrawal, setSelectedWithdrawal] =
    useState<PendingWithdrawalStorage | null>(null);
  ...
  const { addWithdrawal, updateWithdrawal, getWithdrawalsForToken } =
    usePendingWithdrawals();
  ...
  const isValid =
    parseFloat(amount) > 0 && parseFloat(amount) <= parseFloat(sourceBalance);
  ...
  const handleBridge = async () => {
    ...
    try {
      if (direction === 'l1-to-l2') {
        ...
        const l2PublicClient = await import('wagmi/actions').then((m) =>
          m.getPublicClient(config, { chainId: celoSepolia.id }),
        );
    ...
    try {
      ...
      if (result.success) {
        ...
        setSelectedWithdrawal((prev) =>
          prev
            ? { ...prev, status: 'waiting-to-finalize', provenAt: Date.now() }
            : null,
        );
      } else {
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setSelectedWithdrawal((prev) =>
          prev
            ? { ...prev, status: 'finalized', finalizedAt: Date.now() }
            : null,
        );
        onSuccess?.();
      } else {
  ...
  const bridgeSteps: MultistepProgressStep[] =
    direction === 'l1-to-l2'
      ? [
          {
            title: 'Approve Token Transfer',
            description: 'Approving bridge to spend your tokens',
            chain: 'ethereum',
          },
          {
            title: 'Bridge Tokens to L2',
            description: 'Transferring tokens from Ethereum to Celo',
            chain: 'ethereum',
          },
        ]
      : [
          ...
          },
          {
            ...
            description: 'Burning tokens and starting withdrawal',
            chain: 'celo',
          },
        ];
  ...
  return (
    <>
          ...
          <div className='flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg'>
            ...
            <div className='text-xs text-amber-700'>
              <p className='font-medium'>
                L2 → L1 withdrawals require multiple steps:
              </p>
              ...
              {tokenWithdrawals.map((withdrawal) => (
                ...
                >
                      ...
                      <span className='text-sm font-medium'>
                        {formatDisplayNumber(
                          (
                            ...
                            10 ** withdrawal.tokenDecimals
                          ).toString(),
                        )}{' '}
                        {withdrawal.tokenSymbol}
                      </span>
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full ${getStatusColor(withdrawal.status)}`}
                      >
                  ...
                  <div className='text-xs text-gray-500 mt-1 truncate'>
                    TX: {withdrawal.l2TxHash.slice(0, 10)}...
                    {withdrawal.l2TxHash.slice(-8)}
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className='bg-gray-50 rounded-lg p-3 text-xs sm:text-sm'>
          <div className='flex justify-between items-center'>
            ...
            <span className='font-medium'>
              {formatDisplayNumber(sourceBalance)} {symbol}
            </span>
          </div>
        </div>
        ...
        <div className='flex gap-2'>
          {[25, 50, 75, 100].map((pct) => (
            <button
              key={pct}
              onClick={() =>
                setAmount(((parseFloat(sourceBalance) * pct) / 100).toString())
              }
        ...
        {success && (
          <div className='text-green-500 text-xs sm:text-sm'>
            ...
            <p className='text-gray-500 mt-1'>
              Note: Tokens may take a few minutes to appear on the destination
              chain.
            </p>
          </div>
        )}
      ...
      <WithdrawalProgressModal
        ...
        onWaitForProve={() =>
          selectedWithdrawal
            ? waitForReadyToProve(selectedWithdrawal.l2TxHash)
            : Promise.resolve({ ready: false, error: 'No withdrawal' })
        }
        onProve={handleProve}
        onWaitForFinalize={() =>
          selectedWithdrawal
            ? waitForReadyToFinalize(selectedWithdrawal.l2TxHash)
            : Promise.resolve({ ready: false, error: 'No withdrawal' })
        }
        ...
        amount={
          selectedWithdrawal
            ? (
                ...
                10 ** selectedWithdrawal.tokenDecimals
              ).toString()
            : amount || '0'
        }
      />
...
const TransferOwnershipSection = ({
  ...
  const isValid =
    ...
    newOwner.length === 42 &&
    newOwner.toLowerCase() !== currentOwner.toLowerCase();
  const hasPendingTransfer =
    pendingOwner &&
    pendingOwner !== '0x0000000000000000000000000000000000000000';
  const hasL2PendingTransfer =
    l2PendingOwner &&
    l2PendingOwner !== '0x0000000000000000000000000000000000000000';
  ...
  return (
    <>
      <SectionCard title='Transfer ownership'>
        ...
        {(hasPendingTransfer ||
          (isEthereumEnabled && hasL2PendingTransfer)) && (
        ...
        {isOwner && (
          <>
            ...
            <p className='text-[10px] sm:text-xs text-gray-500'>
              Note: The new owner will need to call acceptOwnership() to
              complete the transfer.
            </p>
            <ActionButton
              label='Transfer Ownership'
              onClick={handleTransfer}
              disabled={!isValid || !isOwner}
              loading={isLoading}
            />
          </>
        )}
        ...
        <div className='border border-gray-200 rounded-xl p-3 sm:p-4'>
          ...
          <div className='flex flex-col gap-2'>
            ...
            <div className='flex flex-col sm:flex-row gap-2 sm:gap-2.5'>
              <ReadOnlyField
                label='Total Supply'
                value={formatDisplayNumber(l1TokenManager.totalSupply)}
              />
              <ReadOnlyField
                label='My Balance'
                value={formatDisplayNumber(l1TokenManager.userBalance)}
              />
            </div>
          </div>
        </div>
        ...
        <div className='border border-gray-200 rounded-xl p-3 sm:p-4'>
          ...
          <div className='flex flex-col gap-2'>
            ...
            <div className='flex flex-col sm:flex-row gap-2 sm:gap-2.5'>
              <ReadOnlyField
                label='Total Supply'
                value={formatDisplayNumber(l2TokenManager.totalSupply)}
              />
              <ReadOnlyField
                label='My Balance'
                value={formatDisplayNumber(l2TokenManager.userBalance)}
              />
            </div>
          </div>
        </div>
    ...
    <div className='bg-white rounded-xl sm:rounded-2xl p-3 sm:p-5 flex flex-col gap-3 sm:gap-5'>
      ...
      <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-2'>
        ...
        <div className='flex items-center gap-2'>
          ...
          <div className='flex items-center'>
            {isL2Token ? <CeloIcon /> : <EthereumIcon />}
          </div>
        </div>
      </div>
      ...
      <div className='flex flex-col sm:flex-row gap-2 sm:gap-2.5'>
        ...
        <ReadOnlyField
          label='Total Supply'
          value={formatDisplayNumber(totalSupply)}
        />
        <ReadOnlyField
          label='My Balance'
          value={formatDisplayNumber(userBalance)}
        />
      </div>
    </div>
  );
...
const ContentArea = ({
  ...
  const isOperationLoading =
    isLoading ||
    isSwitchingChain ||
    (l2TokenManager?.isLoading ?? false) ||
    (l2TokenManager?.isSwitchingChain ?? false);

  // Dual-chain transfer ownership for Ethereum Enabled tokens
  const handleTransferOwnershipDual = async (
    newOwner: string,
  ): Promise<{ success: boolean; error?: string }> => {
    if (isEthereumEnabled && l2TokenManager) {
      ...
      if (!l1Result.success) {
        return {
          success: false,
          error: `L1 transfer failed: ${l1Result.error}`,
        };
      }
      ...
      if (!l2Result.success) {
        return {
          success: false,
          error: `L2 transfer failed: ${l2Result.error}. L1 was successful.`,
        };
      }
  ...
  const handleAcceptOwnershipDual = async (): Promise<{
    success: boolean;
    error?: string;
  }> => {
    if (isEthereumEnabled && l2TokenManager) {
      ...
      if (!l1IsOwner) {
        if (l1IsPendingOwner) {
          ...
          if (!l1Result.success) {
            return {
              success: false,
              error: `L1 accept failed: ${l1Result.error}`,
            };
          }
        } else {
      ...
      if (!l2IsOwner) {
        if (l2IsPendingOwner) {
          ...
          if (!l2Result.success) {
            return {
              success: false,
              error: `L2 accept failed: ${l2Result.error}. L1 was successful.`,
            };
          }
        } else {
  ...
  const handlePauseDual = async (
    ...
    if (isEthereumEnabled && l2TokenManager) {
      ...
      if (!l2Result.success) {
        return {
          success: false,
          error: `L2 pause failed: ${l2Result.error}. L1 was paused successfully.`,
        };
      }
  ...
  const handleUnpauseDual = async (
    ...
    if (isEthereumEnabled && l2TokenManager) {
      ...
      if (!l1Result.success) {
        return {
          success: false,
          error: `L1 unpause failed: ${l1Result.error}`,
        };
      }
      ...
      if (!l2Result.success) {
        return {
          success: false,
          error: `L2 unpause failed: ${l2Result.error}. L1 was unpaused successfully.`,
        };
      }
      ...
      case 'admin':
        return (
          <div className='flex flex-col gap-5'>
            ...
            {isEthereumEnabled && (
              <p className='text-xs text-blue-600 bg-blue-50 p-2 rounded-lg'>
                ℹ️ This is an Ethereum Enabled token. Ownership changes will be
                applied to both L1 and L2 tokens.
              </p>
            )}
            <TransferOwnershipSection
              ...
              isOwner={isOwner}
              isPendingOwner={
                isPendingOwner || (l2TokenManager?.isPendingOwner ?? false)
              }
              currentOwner={owner || ''}
...
export default function TokenManager() {
  ...
  const isEthereumEnabled =
    tokenType === 'ethereum-enabled' && !!l2TokenFromParams;
  ...
  const isLoadingToken =
    isL2Token === null && !tokenManagerL2.name && !tokenManagerL1.name;
    ...
    <div className='bg-gray-100 flex flex-col flex-1 min-h-0 w-full p-3 sm:p-6 animate-fade-in items-center'>
      <div className='w-full max-w-[960px]'>
        ...
        <TokenInfoHeader
          ...
          l2TokenAddress={
            isEthereumEnabled ? l2TokenFromParams || undefined : undefined
          }
